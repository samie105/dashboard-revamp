...
export function formatWalletActionError(error: unknown, chain?: string, asset?: string): string {
  ...
  if (code === "BRIDGE_APPROVAL_PENDING") return "USDC approval is still confirming on Arbitrum. We will retry the bridge shortly."
  if (code === "NONCE_STALE" || /nonce too high|invalid transaction nonce/i.test(text)) return "The wallet transaction sequence changed. Refresh and try the bridge again."
