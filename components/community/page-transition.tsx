...
export function CommunityPageTransition({ onComplete }: { onComplete: () => void }) {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      const tl = gsap.timeline({
        onComplete: () => {
          const top = containerRef.current?.querySelector("[data-half='top']") as HTMLElement | null
          const bottom = containerRef.current?.querySelector("[data-half='bottom']") as HTMLElement | null
          if (!top || !bottom) { onComplete(); return }
          const reveal = gsap.timeline({ onComplete })
          reveal.to(top, { yPercent: -100, duration: 0.5, ease: "power4.inOut" }, 0)
          reveal.to(bottom, { yPercent: 100, duration: 0.5, ease: "power4.inOut" }, 0)
        },
      })
  ...
  return (
    ...
    >
      {/* Split halves for the reveal */}
      <div data-half="top" className="absolute inset-0 bottom-1/2 bg-background overflow-hidden">
        <div className="absolute inset-0 bottom-auto h-screen" />
      </div>
      <div data-half="bottom" className="absolute inset-0 top-1/2 bg-background overflow-hidden">
        <div className="absolute inset-0 top-auto h-screen" style={{ transform: "translateY(-50%)" }} />
      </div>
        ...
        {[200, 150, 100, 60].map((size, i) => (
          <div
            ...
            style={{
              ...
              border: `1px solid`,
              borderColor: isDark
                ? `hsl(var(--primary) / ${0.1 + i * 0.08})`
                : `hsl(var(--primary) / ${0.06 + i * 0.05})`,
              boxShadow: isDark
                ? `0 0 ${12 + i * 6}px hsl(var(--primary) / ${0.04 + i * 0.02})`
                : "none",
            }}
          />
        ))}
          ...
          <div
            ...
            style={{
              ...
              background: isDark
                ...
                : `hsl(var(--primary) / ${p.opacity})`,
              boxShadow: isDark ? `0 0 6px hsl(var(--primary) / 0.3)` : "none",
            }}
          />
        ))}
